...
print_imported_functions()
{
    ...
    /*
    *   Main import functions will be in the
    *   .dynsym (dynamic symbols) section, these
    *   will be even if the binary is stripped.
    */
    if (elf_dynsym)
    {
        for (i = 0; i < dynsym_num; i++)
        {
            ...
            if (type == STT_FUNC && elf_dynsym[i].st_value == 0)
            {
                if (elf_dynsym[i].st_name != 0)
                {
                    if (&DynSymbolStringTable[elf_dynsym[i].st_name])
    ...
    /*
    *   Check also if you find functions
    *   in .symtab, in here the imports
    *   will have more information as the
    *   library where they come from.
    *   Example:
